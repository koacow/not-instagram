const posts = [
    {
        id: 1,
        user: "JohnDoe",
        content: "Just had an amazing vacation in Hawaii! 🌴🌺 #vacation #hawaii",
        likes: 100,
        comments: [
            {
                user: "JaneSmith",
                comment: "Wow, looks incredible! 😍"
            },
            {
                user: "MikeJohnson",
                comment: "I'm so jealous! 😩"
            }
        ]
    },
    {
        id: 2,
        user: "EmilyJones",
        content: "Excited to announce the launch of my new website! Check it out: www.emilyjones.com",
        likes: 50,
        comments: [
            {
                user: "DavidBrown",
                comment: "Congratulations! The website looks amazing! 👏"
            },
            {
                user: "SarahWilson",
                comment: "Great job, Emily! Can't wait to explore it! 🎉"
            }
        ]
    },
    {
        id: 3,
        user: "AlexSmith",
        content: "Happy birthday to my best friend! 🎉🎂 #birthday #celebration",
        likes: 200,
        comments: [
            {
                user: "JessicaMiller",
                comment: "Wishing you the happiest birthday ever! 🥳🎈"
            },
            {
                user: "ChrisDavis",
                comment: "Party time! Let's celebrate! 🎉🎊"
            }
        ]
    }
];

export { posts };